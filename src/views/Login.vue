      ...
      <form>
        ...
        <div class="login-verification" v-show="loginPattern">
          <section class="phone">
            ...
            <input ref="phone1" type="tel" maxlength="11" placeholder="请输入手机号" v-model="phone">
        ...
        <div class="login-cipher" v-show="!loginPattern">
          ...
          <section class="code">
            ...
            <img v-if="pwdPattern" src="../assets/images/hide.png" alt="" @click.prevent="showPwd(false)">
            <img v-else src="../assets/images/show.png" alt="" @click.prevent="showPwd(true)">
          </section>
          <section class="captcha">
            ...
            <img ref="captcha" src="http://localhost:3000/api/captcha" alt="" @click.prevent="getCaptcha()">
          </section>
        </div>
...
export default {
  ...
  data(){
    return {
      loginPattern: true, // 登录方式 true代表手机验证码登录 false代表密码登录
      phone: '',  // 手机号
      verifyCode: '', // 手机验证码
      countdown: 0,  // 验证码计时
      username: '', // 账号
  ...
  methods:{
    ...
    // 登录
    async login(){
      if(this.loginPattern){ // 手机验证码登录
        if(!this.phone){ // 手机号为空
          Notify('请输入手机号');
          return;
        }else if(!this.phoneRight){ // 手机号格式不正确
          Notify('请输入正确的手机号');
          return;
        } 
        if(!this.verifyCode){ // 验证码为空
          Notify('请输入验证码');
          return;
        }else if(!(/^\d{6}$/gi.test(this.verifyCode))){ // 验证码格式不正确
          Notify('请输入正确的验证码');
          return;
        }
        // this.result = await verifyCodeLogin(this.phone, this.verifyCode); // 登录
      }else{  // 密码登录
        if(!this.username){ // 用户名/手机/邮箱为空
          Notify('请输入用户名/手机/邮箱');
          return;
        }
        ...
        else if(!this.captcha){ // 验证码为空
          Notify('请输入验证码');
          return;
        }
        // this.result = await pwdLogin(this.username, this.password, this.captcha); // 登录
      }
      if(this.result.status === 200){
        console.log(this.result);
        Notify('登陆成功');
      }else{
        Notify(this.result.message);
        this.getCaptcha();
        return;
      }
      if(!this.result.message.id){  // 没有到获取用户信息
        Notify('登录失败');
        return;
      }else{
